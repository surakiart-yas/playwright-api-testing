...
import { CommentsSchemas } from './schemas'

/** STRUCTURAL assertions — HTTP status + bare-resource schema (GoRest has no envelope). */
export class CommentsValidator extends BaseValidator {
  static async expectCommentSuccess(
    res: APIResponse,
    status: number = HttpStatus.OK,
  ): Promise<void> {
    await this.verify(`comment success — ${status} · bare-resource schema`, async () => {
      BaseValidator.expectStatus(res, status)
      await BaseValidator.expectSchema(res, CommentsSchemas.comment)
    })
  }

  static async expectCommentListSuccess(res: APIResponse): Promise<void> {
    await this.verify('comment list success — 200 · bare-array schema', async () => {
      BaseValidator.expectStatus(res, HttpStatus.OK)
      await BaseValidator.expectSchema(res, CommentsSchemas.commentList)
    })
  }
